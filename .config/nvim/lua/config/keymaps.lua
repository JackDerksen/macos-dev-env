...
M.setup_general = function()
  ...
  -- Indent and de-indent in visual mode using Tab / Shift-Tab
  map("v", "<Tab>", ">gv")
  map("v", "<S-Tab>", "<gv")
  ...
  -- Insert above/below with extra spacing
  map("n", "<leader>o", "o<Esc>o<Esc>O")
  map("n", "<leader>O", "O<Esc>O<Esc>o")

  -- Go to bottom of the screen and center the current line
  map("n", "G", "Gzz")
  ...
  -- Make file executable (commented out for now because it's kind of annoying with trouble)
  -- map("n", "<leader>x", "<cmd>!chmod +x %<CR>", { desc = "Make file executable", silent = true })

  -- Fast exit (at least... it feels faster to me)
  map("n", "<leader>z", "ZZ")
end
